...
TARBALLURL="https://github.com/startlifecoin/startlife/releases/download/1.2.0.0/linux-gnu_1.2.0.0.tar.gz"
TARBALLNAME="linux-gnu_1.2.0.0.tar.gz"
BOOTSTRAPURL=""
BOOTSTRAPARCHIVE=""
BWKVERSION="1.1.0"
...
echo "
 ...
 | the STLC Team CANNOT assist you in repairing  |::
...
echo "" && echo 'Added user "startlife"' && echo ""
sleep 1
...
rm $TARBALLNAME
mv ./startlifed /usr/local/bin
mv ./startlife-cli /usr/local/bin
mv ./startlife-tx /usr/local/bin
rm -rf $TARBALLNAME

# Create .startlife directory
mkdir $USERHOME/.startlife
...
# Create startlife.conf
touch $USERHOME/.startlife/startlife.conf
cat > $USERHOME/.startlife/startlife.conf << EOL
${INSTALLERUSED}
rpcuser=${RPCUSER}
...
masternode=1
addnode=188.120.227.155
addnode=188.120.237.71
addnode=188.120.237.75
addnode=188.120.234.242
addnode=188.120.235.175
EOL
chmod 0600 $USERHOME/.startlife/startlife.conf
chown -R $USER:$USER $USERHOME/.startlife

sleep 1

cat > /etc/systemd/system/startlife.service << EOL
[Unit]
Description=startlifed
After=network.target
[Service]
Type=forking
User=${USER}
WorkingDirectory=${USERHOME}
ExecStart=/usr/local/bin/startlifed -conf=${USERHOME}/.startlife/startlife.conf -datadir=${USERHOME}/.startlife
ExecStop=/usr/local/bin/startlife-cli -conf=${USERHOME}/.startlife/startlife.conf -datadir=${USERHOME}/.startlife stop
Restart=on-abort
[Install]
WantedBy=multi-user.target
EOL
sudo systemctl enable startlife.service
sudo systemctl start startlife.service
